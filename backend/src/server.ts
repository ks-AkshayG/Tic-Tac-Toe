...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("disconnect_room", (room: string, callback: callback) => {
    const index1 = roomArray.indexOf(room);
    roomArray.splice(index1, 1);
    const index2 = roomArray.indexOf(room);
    roomArray.splice(index2, 1);

    callback(true);

    io.to(room).emit("home", true);
