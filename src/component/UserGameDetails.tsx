...
const UserGameDetails = () => {
  ...
  const winner = () => {
    ...
    return data.user_email === userState0.user_email ? userState1.user_name : userState0.user_name
  }
    ...
    <div>
      <div className="flex flex-row justify-around my-[40px]">
        <div className="flex flex-row">
          ...
          <span className="underline text-[30px] text-blue-600">{data?.user_name}</span>
        </div>
        <div className="flex flex-row">
          <p className=" text-[30px]">Opponent:</p>
          <span className="underline text-[30px] text-blue-600">{opponentName()}</span>
        </div>
      </div>
              ...
              data?.user_states.map((state) => {
                ...
                return(
                    <TableRow key={gameState.index}>
                        <TableCell className="text-left">{gameState.user_name}</TableCell>
